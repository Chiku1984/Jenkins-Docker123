def dockerRun = "sudo docker run -it -p 8585:80 -d --name HCL-Hack85 chika1984/myapp:85.1.0"
pipeline {
  agent any
    tools {
        maven 'maven'
      ...
      stages {
        ...
        stage('Build Docker image') {
         ...
         steps {
			sh 'docker build -t chika1984/myapp:85.1.0 .'
		...
		agent { label 'master' }
		 steps {
			...
			sh 'docker push chika1984/myapp:85.1.0'
		} 	
		}
