def dockerRun = "sudo docker run -it -p 8087:8080 -d --name HCL-Hack8087 chika1984/myapp:87.1.0"
pipeline {
  agent any
    tools {
        maven 'maven'
        jdk 'jdk'
    }
      stages {
        stage ('Initialize') {
            steps {
                sh '''
                    echo "PATH = ${PATH}"
					echo 'this **********'
                    '''
            }
        }
		stage ('SonarQube Analysis') {
	      steps {
		  withSonarQubeEnv('sonar-2') {
		  sh "/usr/share/maven/bin/mvn sonar:sonar"
		  }
        }       
       }
	   //stage("SonarQube Quality Gate") {
           // steps {
             // timeout(time: 1, unit: 'MINUTES') {
               // waitForQualityGate abortPipeline: true
              //}
            //}
          //}
        stage ('Build Maven') {
            steps {
                //sh 'mvn -Dmaven.test.failure.ignore=true install' 
				//def mvnHome = tool name: 'maven', type 'maven'
				sh 'mvn clean package'
            }
          }	   
        stage('Build Docker image') {
		  agent { label 'master' }
		  	  
         steps {
			sh 'docker build -t chika1984/myapp:87.1.0 .'

			}
		}	
		
		stage('Push Docker image') {
		agent { label 'master' }
		 steps {
			withCredentials([string(credentialsId: 'DockerHub-Login', variable: 'DockerHubPwd')]) {
            sh "docker login -u chika1984 -p ${DockerHubPwd}"
			}
			sh 'docker push chika1984/myapp:87.1.0'
		} 	
		}
		 stage('Run Docker image on HCL-Hack Server') {
		 steps {
		    sshagent(['HCL-Hack-Code']){ 
			sh "ssh -o StrictHostKeyChecking=no ubuntu@65.1.110.54 ${dockerRun}"		 
		 }
        
}
}
}
}